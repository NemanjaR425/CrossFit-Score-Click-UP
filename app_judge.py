...
st.markdown("""
    <style>
    div.stButton > button:first-child { height: 10em; width: 100%; font-size: 40px !important; background-color: #28a745; color: white; border-radius: 20px; font-weight: bold; }
    .undo button { height: 4em !important; background-color: #6c757d !important; margin-top: 15px; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. GOOGLE SHEETS (The 400 Fix) ---
try:
    conn = st.connection("gsheets", type=GSheetsConnection)
    # Use the clean ID from secrets
    sheet_id = st.secrets["connections"]["gsheets"]["spreadsheet"]
    
    # We add 'usecols' to tell Google exactly what we want, which helps avoid 400 errors
    df = conn.read(
    ...
    # Force convert Athlete_ID to string to prevent decimal points (.0)
    athlete_list = [f"{str(int(row['Athlete_ID']))} - {row['Name']}" for _, row in df.iterrows()]
except Exception as e:
